...
def manual() -> str:
    return "<b>1.</b> На сайте kwork.ru в разделе «Биржа» выбери любимые рубрики, по ним бот будет отслеживать проекты.\n" \
           "<i>(❗️ Важно: в настройках аккаунта Kwork должно быть выбрано «Я продавец»)</i>\n" \
...
def support() -> str:
    return "❗️ Перед обращением в поддержку, проверь, нет ли ответа на твой вопрос в разделе «📜 Инструкция».\n\n" \
